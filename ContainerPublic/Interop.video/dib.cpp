...
void CDIB::Draw(CRect rect, int srcX, int srcY, CDIB *dib, unsigned int flags)
{
	...
	for(int y = 0; y < h; y++)
	{
		...
		for(int x = 0; x < w; x++)
		{
			...
			{
				/*pd[x]->r = (unsigned char)(ps[x]->r * ps[x]->a / 255 + pd[x]->r * pd[x]->a * (255 - ps[x]->a) / (255 * 255));
				pd[x]->g = (unsigned char)(ps[x]->g * ps[x]->a / 255 + pd[x]->g * pd[x]->a * (255 - ps[x]->a) / (255 * 255));
				pd[x]->b = (unsigned char)(ps[x]->b * ps[x]->a / 255 + pd[x]->b * pd[x]->a * (255 - ps[x]->a) / (255 * 255));
				pd[x]->a = (unsigned char)(ps[x]->a + pd[x]->a * (255 - ps[x]->a) / 255);*/
