...
long CIniReader::removeKey(CString strKey, CString strSection)
{
	m_lRetValue = WritePrivateProfileString((LPCTSTR)strSection, (LPCTSTR)strKey, NULL, (LPCTSTR)m_strFileName);
	return m_lRetValue;
}

long CIniReader::removeSection(CString strSection)
{
	m_lRetValue = WritePrivateProfileString((LPCTSTR)strSection, NULL, NULL, (LPCTSTR)m_strFileName);
	return m_lRetValue;
}
...
{
	m_lRetValue = WritePrivateProfileString((LPCTSTR)strSection, (LPCTSTR)strKey, 
                             (LPCTSTR)strValue, (LPCTSTR)m_strFileName);
	return m_lRetValue;
}

long CIniReader::setKey(int intValue, CString strKey, CString strSection)
{
	CString value;
	value.Format(L"%d", intValue);
	return setKey(value, strKey, strSection);
}
