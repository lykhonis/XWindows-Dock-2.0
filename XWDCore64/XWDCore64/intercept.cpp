...
ptrdiff_t SizeOfCode(void *code)
{
	ptrdiff_t last = (ptrdiff_t)code;
	...
	if(last)
	{
		...
		case 0x0800:
			code = (void*)((ptrdiff_t)code + 2);
			break;

		case 0x0c00:
			code = (void*)((ptrdiff_t)code + operandOveride);
			break;
		...
		default:
			{
				...
				case 0x9a:
				case 0xea:
					code = (void*)((ptrdiff_t)code + operandOveride + 2);
					break;

				case 0xc8:
					code = (void*)((ptrdiff_t)code + 3);
					break;
				...
				case 0xa2:
				case 0xa3:
					code = (void*)((ptrdiff_t)code + addrOveride ? 2 : 4);
					break;
				}
			}
			break;
	...
	return (ptrdiff_t)code - last;
}

ptrdiff_t InterceptAPISaveFunction(void *functionAddr, void *functionOldAddr)
{
	ptrdiff_t saveSize = 0;
	void *next = functionAddr;
	while(saveSize < 5)
	{
		ptrdiff_t size = SizeOfCode(next);
		next = (void*)((ptrdiff_t)next + size);
		saveSize += size;
	}
	...
	// 5 bytes
	(*((BYTE*)((ptrdiff_t)functionOldAddr + saveSize))) = 0xe9; // jmp
	(*((ptrdiff_t*)((ptrdiff_t)functionOldAddr + saveSize + 1))) = (ptrdiff_t)next - (ptrdiff_t)functionOldAddr - 5 - saveSize; // relative address
...
bool InterceptAPI(wchar_t *module, char *functionName, void *functionInterceptAddr, void **functionOldAddr)
{
	...
	DWORD oldProtect;
	VirtualProtect(functionAddr, 9, PAGE_EXECUTE_READWRITE, &oldProtect);
	
	void *oldFunction = VirtualAlloc(NULL, 255, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	(*(ptrdiff_t*)oldFunction) = (ptrdiff_t)functionAddr;
	(*((BYTE*)((ptrdiff_t)oldFunction + 8))) = (BYTE)InterceptAPISaveFunction(functionAddr, (void*)((ptrdiff_t)oldFunction + 9));

	(*((BYTE*)functionAddr)) = 0xe9;
	(*((ptrdiff_t*)((ptrdiff_t)functionAddr + 1))) = (ptrdiff_t)functionInterceptAddr - (ptrdiff_t)functionAddr - 9;

	VirtualProtect(functionAddr, 9, oldProtect, &oldProtect);

	(*functionOldAddr) = (void*)((ptrdiff_t)oldFunction + 9);
	return true;
}

bool InterceptAPIRestore(void *functionOldAddr)
{
	void *proc = (void*)(*((ptrdiff_t*)((ptrdiff_t)functionOldAddr - 9)));
	DWORD saveSize = (*((BYTE*)((ptrdiff_t)functionOldAddr - 1)));
	DWORD oldProtect;
	VirtualProtect(proc, 9, PAGE_EXECUTE_READWRITE, &oldProtect);
	memcpy(proc, functionOldAddr, saveSize);
	VirtualProtect(proc, 9, oldProtect, &oldProtect);
	VirtualFree((void*)((ptrdiff_t)functionOldAddr - 9), 255, MEM_RELEASE);
	return true;
}
